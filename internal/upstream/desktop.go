...
//
// # 移植说明（1:1 移植自 workbuddy2api-panel/internal/upstream/desktop.go）
//
// 本文件**逐字移植**，包括全部注释里的实测证据（日期 / 账号名 / 抓包行号）——
// 那是这套判据唯一的来源记录，改写成"自己的话"等于把证据丢掉。
//
// 仅有的适配：
//
//	模块路径   github.com/linguo2625469/workbuddy2api-panel/internal/auth
//	         → workbuddy2api/internal/auth
//	desktopUA 在本文件定义（B 在 client 段定义）
//	WebBaseCN 落到 Client 字段上（本仓库新增该字段，见 client.go 的注释）
//
// 其余（端点、请求头、事件载荷、字段名、顺序）**一个字节都没动**。
//
// # 原始来源
//
// 2026-09-12 Sunny 抓包实测（data/desktop-task-protocol.md）。桌面端点亮
...
const (
	...
	// defaultWebBaseCN 官网域默认值（webBase 的回落目标）。
	defaultWebBaseCN = "https://www.workbuddy.cn"
)
