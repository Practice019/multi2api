...
//	Token 到期     **上一轮补上的** —— 见本文件顶部注释
//	福利           **上一轮补上的** —— 今天领过没有 / 还能领几项
//	操作           行内动作（领取福利 / 禁用 / 移除…）
//
// # 为什么「上游」列**又加回来了**（用户实测提的要求）
//
// 原先这里是**刻意不含**「上游」的，理由是"分组后同组同名，冗余"。
// 那条理由在**只看本组**时成立，但它忽略了一件事：**同一列在不同上游的
// 表里位置不同**。workbuddy 走核心默认列集（`DefaultAccountColumns()`），
// 它的第一列就是「上游」；codearts 这份从「昵称」开头 —— 于是同一个面板里
// 两张表的**第一列不是同一个东西**，横向看过去对不齐。
//
// 用户的诉求正是这个：「和其他上游的表格统一一下第一个列」。
//
// 所以现在 codearts 与 workbuddy **首列一致**（都是 provider），
// 且这一列的 id 是同一个（`gateway.AccountColProvider`）——
// 前端只按 id 查渲染器，两家共用同一个单元格，不存在第二份实现。
//
//	"分组标题也写着上游名"这件事**不构成省略它的理由**：
//	 账号行在筛选、复制、导出时是**脱离分组标题**存在的（见 webui.html
//	 里 `accountRow` 的注释：这一列让单行自证归属）。
//	 分组标题给的是整体结构，这一列给的是单行的归属，两者不重复。
//
// 刻意**不含**：
//
//	「今日签到」   它没有这个能力位
